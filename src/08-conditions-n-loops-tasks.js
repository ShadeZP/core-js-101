...
function isInsideCircle(circle, point) {
  ...
  if (((point.x - center.x) ** 2 + (point.y - center.y) ** 2) < (radius ** 2)) return true;
  return false;
}
...
function getDigitalRoot(num) {
  const res = num.toString().split('').reduce((acc, curr) => +acc + +curr, 0);
